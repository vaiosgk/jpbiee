...
var iom = iom || (function () {
  ...
  function loadSurvey() {
    szmvars = result.st+"//"+result.pt+"//"+result.cp+"https://via_SZMNG";
    ...
    if (sv && !surveyCalled && result.sv !== "ke" && (result.sv === "in" || result.sv === "mo" || result.sv === "i2" )) {
      ...
      if (loaded==0 && result.sv === "in") document.write("<script src='"+window.location.protocol+'https://qs.ioam.de/?'+((window.szmvars)?szmvars:"")+"'></script>");
      if (loaded==0 && result.sv === "i2") {
        if (IAMQSElement) {
          IAMQSElement.parentNode.removeChild(IAMQSElement);
        }
        IAMQSElement = createScriptTag(window.location.protocol+'https://qs.ioam.de/?'+((window.szmvars)?szmvars:""));
      }
      if (loaded==0 && result.sv === "mo") {
        if (IAMQSElement) {
          IAMQSElement.parentNode.removeChild(IAMQSElement);
        }
        IAMQSElement = createScriptTag(window.location.protocol+'https://mqs.ioam.de/?'+((window.szmvars)?szmvars:""));
      }
    }
  }
